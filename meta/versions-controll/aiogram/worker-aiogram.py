...
from aiogram.types import ParseMode, ChosenInlineResult
...
"""
        ...
        future = bot_send_message(text, type_='bot-exception',
...
# Since v1.0.9.
async def is_participant(user_id, chat_id=HEAD_CHAT) -> bool:
    """User is participant of chat with id `chat_id`, return bool."""
    try:
        return (await bot.get_chat_member(chat_id, user_id)).is_chat_member()
    except:
        return False


# Since v1.0.9.
async def _add_user(user_id: Union[int, str]) -> typing.NoReturn:
...
# Since v1.0.9.
async def _make_move(message, letter: str, mentioned: Iterable) \
    -> Optional[tuple[str, str]]:
...
async def make_move(message) -> typing.NoReturn:
    ...
    * feature_exists('teach_word')
            await message.reply(answer_msg, parse_mode='html')
            return
        ...
        if int(order[current]) == BOT_ID:
            ...
            answer, msg = await _make_move(message, letter, mentioned)
...
# Since v1.0.9.
async def words_skip_move(message: types.Message) -> typing.NoReturn:
    ...
    try:
        ...
        result = await _make_move(message, letter, mentioned)
        assert result
    ...
    if int(order[current]) == BOT_ID:
        ...
        # await perform_bots_move()  # See def at version telethon v1.0.9.
...
async def do_action(message: types.Message):
    ...
    try:
        ...
        await message.reply(msg, parse_mode=MARKDOWN_V2)
...
async def start(message):
    ...
    if 'test_start_message' in globals() and re.fullmatch(p, message.text):
        result = await test_start_message(message)
...
"""
    ...
    await message.answer(msg, parse_mode=MARKDOWN_V2,
...
"""
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[[help_message]])
    await message.answer(msg, parse_mode=MARKDOWN_V2, reply_markup=keyboard)
...
где искомое слово, либо `/meaning слово`."
    ...
    result = await get_word_and_meaning(pattern, message)
    if not result:
...
"""
    ...
    if is_private(message):
        ...
        async def user_id_(e: 'Entity') -> Optional[int]:
            ...
            if e.type == 'text_mention':
                ...
                for user_id in users:
                    # TODO Test, do it
                    try:
                        _user = (await bot.get_chat_member(chat.id,
                                                           user_id)).user
...
# Since v1.0.9.
@dp.message_handler(regexp=commands('greeting', 'new_greeting') + '.*')
async def manage_greeting(message):
...
"""
    ...
    if not is_new_greeting:
        ...
        if match(del_pattern):
            ...
            try:
                ...
                await message.reply(greeting_not_found_text)
                deleted = False
    ...
    if case == 'delete':
        ...
        await message.reply("Приветствие добавлено.")
...
async def greet_new_chat_members(message: types.Message):
    ...
    *****
    ...
    try:
        ...
        # Executing `await bot.get_chat_member(...)` failed.
        pass
    ...
    if chat_id == SPEAK_CHAT:
        ...
        if should_ban:
            ...
            for u in should_ban:
                ...
                await bot.ban_chat_member(chat.id, u.id, until_date=0)
...
# Since v1.0.9.
@dp.inline_handler(lambda inline_query: 0 < len(inline_query.query) <= 255)
...
# Since v1.0.9.
@dp.inline_handler(lambda inline_query: not inline_query.query)
...
# Since v1.0.9.
async def realise_move(message) -> Optional[int]:
...
async def chosen_inline_query_handler(chosen_inline_query: ChosenInlineResult):
    ...
    """
    msg = dedent(msg)
    await bot_send_message(msg, chat_id=LOGGING_CHAT_INLINE_FEEDBACK,
        parse_mode='html')
...
def main():
    """Run the bot."""
    print("Version: aiogram. Running the bot.")
